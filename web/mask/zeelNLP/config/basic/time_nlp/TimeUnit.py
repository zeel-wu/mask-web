...
import arrow
import copy
from web.mask.zeelNLP.config.basic.time_nlp.TimePoint import TimePoint
from web.mask.zeelNLP.config.basic.time_nlp.RangeTimeEnum import RangeTimeEnum

try:
    from web.mask.zeelNLP.config.basic.time_nlp.LunarSolarConverter.LunarSolarConverter import *
except:
    from web.mask.zeelNLP.config.basic.time_nlp.LunarSolarConverter import *
...
class TimeUnit:
    ...
    def norm_sethour(self):
        ...
        if match is not None:
            ...
            # logging.info('first', self.tp.tunit[3] )
            # 处理倾向于未来时间的情况
            self.preferFuture(3)
    ...
    def preferFuture(self, checkTimeIndex):
        ...
        # logging.info(time_arr)
        # logging.info(self.tp.tunit)
        if self.tp.tunit[0] == -1:
            self._noyear = True
        else:
            self._noyear = False
    ...
    def checkContextTime(self, checkTimeIndex):
        ...
        # logging.info('comparision time is ',self.tp_origin.tunit[checkTimeIndex])
