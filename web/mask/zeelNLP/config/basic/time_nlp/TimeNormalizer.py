...
from web.mask.zeelNLP.config.basic.time_nlp.StringPreHandler import StringPreHandler
from web.mask.zeelNLP.config.basic.time_nlp.TimePoint import TimePoint
from web.mask.zeelNLP.config.basic.time_nlp.TimeUnit import TimeUnit
...
class TimeNormalizer:
    ...
    def __timeEx(self):
        ...
        for i in range(0, rpointer):
            ...
            # res[i].tp.tunit[3] = -1
            contextTp = res[i].tp
            # logging.info(self.nowTime.year)
            # logging.info(contextTp.tunit)
